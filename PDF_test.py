...
class MainFrame(wx.Frame):
    ...
    def execOCR(self):
        ...
        for [box, p] in self.AreaBox:
            ...
            im2 = self.pil2cv(im_crop)
            # plt.imshow(im2)
            # plt.show()
            builder = pyocr.builders.TextBuilder(tesseract_layout=6)
            builder_word = pyocr.builders.WordBoxBuilder(tesseract_layout=6)
            # builder = pyocr.builders.LineBoxBuilder(tesseract_layout=6)
            result = tool.image_to_string(im_crop, lang="jpn", builder=builder)
            result_word = tool.image_to_string(im_crop, lang="jpn", builder=builder_word)

            for res in result_word:
                cv2.rectangle(im2, res.position[0], res.position[1], (0, 0, 0), 2)
                # cv2.rectangle(img, (x, y + lh1[i]), (x + w, y + lh1[i] + h), (0, 255, 0), 3)
