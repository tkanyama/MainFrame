...
FRAME_WIDTH = 1500  # メインパネルの幅
FRAME_HEIGHT = 823  # メインパネルの高さ
...
THUMBNAIL_PANEL_HEIGHT = FRAME_HEIGHT  # サムネイルパネルの幅
...
class MainFrame(wx.Frame):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.image_dir = self.home + "/BOX_IMAGE"
        ...
        self.para_dir = self.home + "/PDF_DATA"
        ...
        self.boxText = wx.StaticText(self.ctrl_panel, wx.ID_ANY,
                                     '[ ボックス ]', pos=(THUMBNAIL_PANEL_WIDTH + BUTTON_WIDTH * 1.2, 5))
        ...
        # 画像を保存するボタン
        self.img_save_button = wx.Button(self.ctrl_panel, wx.ID_ANY, 'イメージ保存',
                                     size=(BUTTON_WIDTH * 1.25, BUTTON_HEIGHT),
                                     pos=(THUMBNAIL_PANEL_WIDTH + PDF_PANEL_WIDTH , CONTROL_PANEL_HEIGHT / 2))
        self.Bind(wx.EVT_BUTTON, self.img_save_button_handler1, self.img_save_button)  # イベントを設定
        # self.Bind(wx.EVT_BUTTON, self.box_DEL_buttonn_handler, self.ocr_button1)  # イベントを設定
        self.v_layout1.Add(self.img_save_button)

        # 形態解析を開始するボタン
        self.analysis_button = wx.Button(self.ctrl_panel, wx.ID_ANY, 'データ作成',
                                         size=(BUTTON_WIDTH, BUTTON_HEIGHT),
                                         pos=(THUMBNAIL_PANEL_WIDTH + PDF_PANEL_WIDTH + BUTTON_WIDTH * 1.5, 0))
        ...
        self.result_save_button = wx.Button(self.ctrl_panel, wx.ID_ANY, 'データ保存',
                                            size=(BUTTON_WIDTH, BUTTON_HEIGHT),
                                            pos=(THUMBNAIL_PANEL_WIDTH + PDF_PANEL_WIDTH + BUTTON_WIDTH * 2.75, 0))
        ...
        # =======================[ サブ　パネル ]=========================================
        self.panel_height1 = self.frame_h - self.bar_w - CONTROL_PANEL_HEIGHT
        ...
        self.pdf_scroll_panel = scrolled.ScrolledPanel(self.subPanel1, wx.ID_ANY, pos=(THUMBNAIL_PANEL_WIDTH, 0),
                                                       size=(PDF_PANEL_WIDTH, PDF_PANEL_HEIGHT), style=wx.DOUBLE_BORDER)
        ...
        self.result_text1 = wx.TextCtrl(self.Text_panel_1, wx.ID_ANY, pos=(0, 0),
                                        size=(TEXT_PANEL_WIDTH1, TEXT_PANEL_HEIGHT1 * 0.9),
    ...
    def load_data(self):
        if os.path.exists(self.para_dir) == False:
    ...
    def img_save_button_handler1(self, event):
        self.imageSave()
    ...
    def imageSave(self):
        ...
        if os.path.exists(self.image_dir) == False:  # ディレクトリーが存在しない場合作成する。
        ...
        if name1[-2:] == '-1':
            self.image_name = name1[:-2] # + '.png'
        else:
            self.image_name = name1 # + '.png'

        self.iDir2 = os.getcwd()  # 変更後のディレクトリーの読込

        if os.path.exists(self.image_name) == False:  # ディレクトリーが存在しない場合作成する。
        ...
        for [box, p] in self.AreaBox:
            ...
            if kind == 0:
                title_n += 1
                fname = self.image_name + '_title_{}'.format(title_n) + '.png'
            elif kind == 1:
                date_n += 1
                fname = self.image_name + '_date_{}'.format(date_n) + '.png'
            elif kind == 2:
                name_n += 1
                fname = self.image_name + '_name_{}'.format(name_n) + '.png'
            elif kind == 3:
                doc_n += 1
                fname = self.image_name + '_doc_{}'.format(doc_n) + '.png'

            im_crop.save(self.image_name + '/' + fname, quality=300)
            # imwrite(fname, im_crop)
